    ...
    public class TagData : ITagData
    {
        ...
        public double GetTagTotalSpend(int tagId, DateTime startDate, DateTime endDate, bool isIncome)
        {
            ...
            try
            {
                ...
                strCmdText += "WHERE Transactions.IsIncome = 0 AND (Transactions.Date >= CONVERT(DATETIME, '" + startDate.ToString("yyyy/MM/dd") + " 00:00:00')) AND (Transactions.Date <= CONVERT(DATETIME, '" + endDate.ToString("yyyy/MM/dd ") + " 23:59:59')) AND Transaction_Tags.TagId = " + tagId.ToString();
